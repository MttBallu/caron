...
Unit tests check snapshot mismatch, parameter use, target-kind filtering, and distinct support rows. The live suite collects **three independently reported tests**: `C01_learning_without_activity`, `C02_activity_without_learning`, and the existing fixture round-trip and rollback test. Each C01/C02 test projects its own accepted realisation, checks exact bindings and support IDs, and restores the whole-career fixture in `finally`; the third test also finishes with the whole-career fixture. A single case can therefore be rerun without relying on test order. Against the dedicated disposable Community `2026.09.0` container, after pulling the experiment branch:
